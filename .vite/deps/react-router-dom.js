import { __toESM, require_react } from './chunk-BHZAVQOK.js';
...
function getSubmissionFromNavigation(navigation) {
  let { formMethod, formAction, formEncType, text, formData, json: json3 } = navigation;
    ...
    (arg) => {
      ...
      return shouldBlock({
        ...
        nextLocation: _extends2({}, nextLocation, {
          pathname:
            stripBasename(nextLocation.pathname, basename) || nextLocation.pathname,
        }),
        historyAction,
      });
    },
...
var NavLink = React2.forwardRef(function NavLinkWithRef(_ref5, ref) {
  ...
  if (typeof classNameProp === 'function') {
    ...
    className = [classNameProp, isActive ? 'active' : null, isPending ? 'pending' : null]
      .filter(Boolean)
      .join(' ');
  }
  let style =
...
function useSubmitFetcher(fetcherKey, fetcherRouteId) {
  let { router } = useDataRouterContext2(DataRouterHook2.UseSubmitFetcher);
  let { basename } = React2.useContext(NavigationContext);
  return React2.useCallback(
    function (target, options) {
      if (options === void 0) {
